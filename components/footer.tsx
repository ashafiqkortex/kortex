    ...
    <footer className="relative border-t border-[var(--border)] mt-32">
      ...
      <Container size="wide" className="relative py-16">
        <div className="grid gap-12 md:grid-cols-[1.5fr_1fr_1fr_1fr]">
          <div>
            <Logo />
            <p className="mt-5 text-[13.5px] leading-relaxed text-[var(--muted)] max-w-xs">
              ...
              <ul className="space-y-2.5">
                {section.links.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="text-[13.5px] text-[var(--foreground-dim)] hover:text-[var(--accent)] transition-colors"
                    >
                      {link.label}
                    </Link>
                  </li>
        ...
        <div className="mt-16 pt-6 border-t border-[var(--border)] flex flex-col md:flex-row items-start md:items-center justify-between gap-4 text-[12px] text-[var(--muted-2)]">
