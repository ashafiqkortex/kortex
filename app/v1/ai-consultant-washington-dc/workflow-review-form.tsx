...
// Two-step Free Workflow Review request for the Washington DC local page.
...
export function WorkflowReviewForm() {
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const message = [
      "FREE 20-MINUTE WORKFLOW REVIEW REQUEST — /ai-consultant-washington-dc",
      "",
      `Website:            ${d.site || "—"}`,
      `City and state:     ${d.cityState || "—"}`,
      `Meeting preference: ${d.meeting || "—"}`,
      "",
      "Workflow that is slow, repetitive or difficult to control:",
      d.workflow || "—",
      "",
    ...
    return (
      <div className="form-sent" role="status">
        ...
        <p>
          ...
          arrange the 20-minute Workflow Review and confirm whether an online or in-person
          conversation makes the most sense. If it&apos;s urgent, call{" "}
  ...
  return (
    <form className="v1-form" onSubmit={submit} noValidate>
      <p className="wdc-step">STEP {step} OF 2 — {step === 1 ? "THE WORKFLOW" : "CONTACT AND MEETING"}</p>
      <p className="form-index">WHICH WORKFLOW SHOULD WE LOOK AT?</p>
      ...
      <div data-step="1" style={step === 1 ? undefined : { display: "none" }}>
        ...
        <label><span className="field-label">City and state <em aria-hidden="true">*</em></span>
          <input name="cityState" required autoComplete="address-level2" />
        </label>
        <label><span className="field-label">Which workflow is slow, repetitive or difficult to control? <em aria-hidden="true">*</em></span>
      ...
      <div data-step="2" style={step === 2 ? undefined : { display: "none" }}>
        ...
        <fieldset className="wdc-meeting">
          <legend className="field-label">Meeting preference</legend>
          <label><input type="radio" name="meeting" value="Online" /><span>Online</span></label>
          <label><input type="radio" name="meeting" value="In person when practical" /><span>In person when practical</span></label>
          <label><input type="radio" name="meeting" value="No preference" defaultChecked /><span>No preference</span></label>
        </fieldset>
