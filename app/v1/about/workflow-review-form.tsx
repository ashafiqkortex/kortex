...
// Two-step Free Workflow Review request for /about — the same offer and form
// used on the principal conversion pages. Submits to the existing
...
export function WorkflowReviewForm() {
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const message = [
      "FREE WORKFLOW REVIEW REQUEST — /about",
      "",
      `Website:  ${d.site || "—"}`,
      "",
      ...
      d.workflow || "—",
      "",
      "Who is involved in it today?",
      d.involved || "—",
      "",
      "What happens when it breaks or falls behind?",
      d.breaks || "—",
    ].join("\n");
    try {
    ...
    return (
      <div className="form-sent" role="status">
        ...
        <p>
          ...
          arrange the 20-minute Workflow Review and confirm who from Kortex will join. If
  ...
  return (
    <form className="v1-form" onSubmit={submit} noValidate>
      <p className="abt-form-step">STEP {step} OF 2 &mdash; {step === 1 ? "THE WORKFLOW" : "CONTACT"}</p>
      <p className="form-index">BOOK MY FREE WORKFLOW REVIEW</p>
      ...
      <div data-step="1" style={step === 1 ? undefined : { display: "none" }}>
        ...
        <label><span className="field-label">Which workflow is slowing the company down? <em aria-hidden="true">*</em></span>
          <textarea name="workflow" rows={2} required
            placeholder="Scheduling, job paperwork, approvals, invoicing&hellip;" />
        </label>
        <label><span className="field-label">Who is involved in it today? <em aria-hidden="true">*</em></span>
          <textarea name="involved" rows={2} required
            placeholder="The owner, the office, field crews, a bookkeeper&hellip;" />
        </label>
        <label><span className="field-label">What happens when it breaks or falls behind? <em aria-hidden="true">*</em></span>
          <textarea name="breaks" rows={3} required
            placeholder="A delayed invoice, a missed job, a last-minute scramble&hellip;" />
        </label>
