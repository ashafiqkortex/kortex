...
export function FitReviewForm() {
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const message = [
      "SOFTWARE FIT REVIEW REQUEST — /servicetitan-alternative",
      "",
      `Website:            ${d.site || "—"}`,
      `Contractor type:    ${d.contractorType || "—"}`,
      `Using ServiceTitan: ${d.usingSt || "—"}`,
      "",
      `Workflow to improve:`,
      d.workflow || "—",
      "",
      ...
      d.manual || "—",
      "",
      `Technicians / field users: ${d.techCount || "—"}`,
  ...
  return (
    <form className="v1-form" onSubmit={submit} noValidate>
      <p className="st-step">STEP {step} OF 2 — {step === 1 ? "YOUR CURRENT DECISION" : "SCALE AND CONTACT"}</p>
      ...
      <div data-step="1" style={step === 1 ? undefined : { display: "none" }}>
        ...
        <label><span className="field-label">Type of contractor <em aria-hidden="true">*</em></span>
          <input name="contractorType" required placeholder="Mechanical, electrical, plumbing, GC, other trade…" />
        </label>
        <fieldset style={{ border: 0, padding: 0, margin: 0 }}>
          <legend className="field-label" style={{ padding: 0 }}>Are you currently using ServiceTitan?</legend>
          <div className="st-radio-row">
            <label><input type="radio" name="usingSt" value="Yes" required /> YES</label>
            <label><input type="radio" name="usingSt" value="No" /> NO</label>
            <label><input type="radio" name="usingSt" value="Evaluating it" /> EVALUATING IT</label>
          </div>
        </fieldset>
        <label><span className="field-label">Which workflow are you trying to improve? <em aria-hidden="true">*</em></span>
          <textarea name="workflow" rows={3} required
            placeholder="Service-to-invoice, dispatch, certified payroll, project approvals, change work…" />
        </label>
        <label><span className="field-label">What does the current process force your team to do manually?</span>
          <textarea name="manual" rows={3}
            placeholder="Retyping, chasing approvals, reconciling spreadsheets…" />
        </label>
      ...
      <div data-step="2" style={step === 2 ? undefined : { display: "none" }}>
        <div className="form-row">
          <label><span className="field-label">Technicians or field users (optional)</span>
            <input name="techCount" inputMode="numeric" />
          </label>
          <label><span className="field-label">Current software (optional)</span>
            <input name="software" placeholder="QuickBooks, spreadsheets, ServiceTitan…" />
          </label>
        </div>
        <div className="form-row">
