...
// Two-step Free Workflow Review request for /ai-agents-for-business — the same
// route and offer as /ai-agent-development and /ai-consulting per the doc's
// internal routing note. Submits to the existing /api/contact route: the
// structured answers are composed into its `message` field so no shared code
// changes. CAREFUL: that route treats a non-empty `website` field as its bot
// honeypot — the real website URL is therefore named `site` here, and
// `website` stays the hidden honeypot.
export function WorkflowReviewForm() {
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const message = [
      "FREE WORKFLOW REVIEW REQUEST — /ai-agents-for-business",
      "",
      `Website:  ${d.site || "—"}`,
      "",
      "What recurring work keeps getting delayed, missed or chased?",
      d.work || "—",
      "",
      "Who handles it today?",
      d.who || "—",
      "",
      "What happens when it slows down or goes wrong?",
      d.breaks || "—",
    ].join("\n");
    try {
  ...
  return (
    <form className="v1-form" onSubmit={submit} noValidate>
      {/* The doc's form heading renders as the section h2 beside this form. */}
      <p className="afb-form-step">STEP {step} OF 2 &mdash; {step === 1 ? "THE WORKFLOW" : "CONTACT"}</p>
      ...
      <div data-step="1" style={step === 1 ? undefined : { display: "none" }}>
        ...
        <label><span className="field-label">What recurring work keeps getting delayed, missed or chased? <em aria-hidden="true">*</em></span>
          <textarea name="work" rows={2} required
            placeholder="Preparing completed work for billing, compliance records, quote follow-up&hellip;" />
        </label>
        <label><span className="field-label">Who handles it today? <em aria-hidden="true">*</em></span>
          <input name="who" required placeholder="Office manager, PM, bookkeeper&hellip;" />
        </label>
        <label><span className="field-label">What happens when it slows down or goes wrong? <em aria-hidden="true">*</em></span>
